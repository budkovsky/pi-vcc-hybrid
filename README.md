...
- **CPU/GPU**: embeddings run on **CPU by default** (`QMD_FORCE_CPU=1`) — the GPU path has been observed to cause CUDA OOM contention on shared machines. Set `semantic.gpu: "force"` to opt in. The one-time model load (~30–60 s cold) happens off the turn path at session start.
...
For the inherited VCC config (thresholds, debug, etc.), see [Config](#config) below.
