...
import { fabricOperationsOf, fabricResultText } from "../src/core/fabric-trace";
...
describe("pi-fabric nested operations", () => {
  it("reads current durable execution traces", () => {
    const operations = fabricOperationsOf(traceDetails);
    ...
    expect(fabricResultText(operations[0]!)).toBe("export const value = 1;");
    expect(fabricResultText(operations[1]!)).toBe("old text not found");
  });

  it("reads legacy audit details", () => {
    expect(fabricOperationsOf({
      audits: [{
        ref: "pi.write",
